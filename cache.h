...
class TWO_LEVEL_CACHE
{
  ...
  private:
    ...
    StreamBufferModule<ADDRINT> *streambuffer;
    ...
    _l2_prefetch_lines(l2PrefetchLines)
{
    ...
    streambuffer = new StreamBufferModule<ADDRINT>(5,5,64);
...
UINT32 TWO_LEVEL_CACHE<SET>::Access(ADDRINT addr, ACCESS_TYPE accessType)
{
    ...
    cycles = _latencies[HIT_L1]; // _latencies[MISS_L1] not added
	...
	//
	strmBufferHit = streambuffer->access(addr); // Check if our data lies in a stream buffer.
	if(!strmBufferHit) 
	{
